...
import { tableNames, ConfiguratorProjectType } from "@/utils/supabase/customTypes";

interface ConfiguratorProjectTypeFormValues {
  name: string;
  key: string;
  description: string;
  icon: string;
...
export const useConfiguratorProjectTypes = () => {
  const [projectTypes, setProjectTypes] = useState<ConfiguratorProjectType[]>([]);
  ...
  const createProjectType = async (projectType: ConfiguratorProjectTypeFormValues): Promise<boolean> => {
    try {
      setError(null);
      ...
      const newProjectType: ConfiguratorProjectType = {
        id: data[0].id,
        name: data[0].name,
        key: data[0].key,
  ...
  const updateProjectType = async (id: string, projectType: ConfiguratorProjectTypeFormValues): Promise<boolean> => {
    try {
      setError(null);
  ...
  const reorderProjectTypes = async (reorderedItems: ConfiguratorProjectType[]): Promise<void> => {
    try {
      setError(null);
...
export type { ConfiguratorProjectTypeFormValues };
